"""
Layer8 SummaryAgent — Hermes 五环

职责：会话结束时提炼记忆、创建/改进 Skill、用户建模
层级位置：L8（学习进化层，在出题闭环之外独立运行）

Hermes 五环机制：
1. 记忆策划：从会话数据中筛选 ≤5 条高价值信息
2. Skill 创建：将高质量出题模式蒸馏为可复用 Skill
3. Skill 自改进：更新 Skill 的步骤、偏好、陷阱
4. 跨会话召回：FTS 检索最相关记忆
5. 用户建模：更新能力估计、行为模式、偏好冲突
