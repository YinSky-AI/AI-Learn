"""
backend/app/ai/prompts/quality_check.py

质量检查 Prompt 模板

本模板用于 L5 QualityCheckAgent，对单道题目执行四维快速质量检查。
通过结构化的检查清单和评分标准，确保检查结果的一致性和可解释性。
...
检查维度：
1. 答案正确性（权重40%）：验证答案绝对正确、无歧义
2. 适龄性（权重25%）：内容适合目标年龄段，题干长度合理
3. 难度匹配（权重20%）：实际难度与预期一致
4. 重复度（权重15%）：题目有创新性，不过于模板化
"""
