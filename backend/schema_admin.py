...
def _safe_integrity_detail(exc: Exception) -> str | None:
    """Expose only PostgreSQL diagnostic identifiers, never statements or values."""
    ...
    if diagnostic is None:
        return None
    fields: list[str] = []
    for label, attribute, pattern in (
        ("sqlstate", "sqlstate", r"[0-9A-Z]{5}"),
        ("constraint", "constraint_name", r"[A-Za-z_][A-Za-z0-9_$]{0,127}"),
        ("table", "table_name", r"[A-Za-z_][A-Za-z0-9_$]{0,127}"),
        ("column", "column_name", r"[A-Za-z_][A-Za-z0-9_$]{0,127}"),
    ):
...
POST_BASELINE_COLUMNS = {
    ...
    ("primary", "knowledge_nodes"): {"code"},
    ("primary", "answers"): {"solution_steps", "student_confidence"},
...
def main() -> int:
    ...
    try:
        ...
        integrity_detail = _safe_integrity_detail(exc)
        if integrity_detail is not None:
            print(f"Schema integrity detail: {integrity_detail}", file=sys.stderr)
