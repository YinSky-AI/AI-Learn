...
def test_autogenerate_failure_detail_exposes_schema_drift_without_credentials():
    detail = _safe_autogenerate_detail(
        AutogenerateDiffsDetected(
            "New upgrade operations detected: [('add_column', 'diagnosis_jobs', 'trace_id')]"
        )
    )

    assert "diagnosis_jobs" in detail
