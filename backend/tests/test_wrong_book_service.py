...
async def test_new_answer_is_flushed_before_wrong_event_is_written(monkeypatch):
    ...
    question = SimpleNamespace(
        question_type="CHOICE", question_body="解方程 x+1=2。",
        correct_answer="A", explanation="解析", id=question_id,
        knowledge_node_rel=SimpleNamespace(title="知识点", subject_code="数学"),
    )
    db = _SubmissionSession(learning_session, None, question, None, None)
...
async def test_retried_answer_id_returns_existing_result_without_new_side_effects(monkeypatch):
    ...
    existing_diagnosis_job = SimpleNamespace(id=uuid.uuid4(), status="queued")
    db = _SubmissionSession(
        learning_session, existing, question, None, existing_diagnosis_job
    )
