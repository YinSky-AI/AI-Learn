...
type PracticeQuestion = { id: string; question_text: string; options?: Array<{ key: string; value: string }>; correct_answer: string; subject?: string; difficulty?: string };

export default function WrongBookPracticePage() {
  const subject = typeof window === "undefined" ? null : new URLSearchParams(window.location.search).get("subject");
  ...
  useEffect(() => { void (async () => {
    ...
    if (!token || TokenManager.isTokenExpired(token)) { setAuthRequired(true); return; }
  ...
  const correct = useMemo(() => submitted && answer.trim().toUpperCase() === question?.correct_answer.trim().toUpperCase(), [answer, question, submitted]);
